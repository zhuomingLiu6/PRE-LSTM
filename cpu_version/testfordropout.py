...
def train(**kwargs):
    ...
    # define the model
    model = TrainingModel_Vec_Dropout(len(word2ix_train), len(word2ix_fix), 200, 400,0.1)
...
def multi_gen(**kwargs):
    ...
    data, word2ix_train, ix2word_train, word2ix_fix, ix2word_fix = load_data(opt.parsed_data_path)
    model = TrainingModel_Vec_Dropout(len(word2ix_train), len(word2ix_fix), 200, 400,0.3)
