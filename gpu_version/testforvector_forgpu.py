...
class TrainingModel(nn.Module):
    ...
    def forward(self, input, hidden=None, mark=None, num=None):
        if mark == None:
	        ...
	        #  ["<START>"] + 部门 + 关键词 + 表格 + ["<EOP>"]
	        # 训练数据应该是 ["<START>"] + 部门 + 关键词 + 表格 
	        #检查行数是不是有问题
	       
            embedschange = self.embeddings(t.cat([input[0:2],input[3:]],dim = 0))
	        # size: (seq_len,batch_size,embeding_dim) tensor 的第二行由 fixembeding来
            embedsfix = self.embeddingsfix(input[2:3])
	        #拼接两个embedding,将embeddingfix放回到第一个embedschange的第二行
            embeds=t.cat([embedschange[0:2],embedsfix,embedschange[2:]],dim = 0)

	        
	        # output size: (seq_len,batch_size,hidden_dim) 将拼接之后的结果输入神经网络
...
def train(**kwargs):
    ...
    # 模型定义
    model = TrainingModel(len(word2ix_train), len(word2ix_fix), 200, 400)
    ...
    t.save(model.state_dict(), '%s_%s.pth' % ("testtestingfix_arg", epoch))
